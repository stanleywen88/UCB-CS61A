...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        outcome.append(dice())
        num_rolls -= 1
    return sum(outcome) if 1 not in outcome else 1
    # END PROBLEM 1
...
def hefty_hogs(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    # Get 1 point if current score is 0
    if opponent_score == 0:
        return 1
    ...
    while opponent_score > 0:
        ...
        opponent_score //= 10
        print("DEBUG", player_score, opponent_score)
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def helper(*args): # the function will take all arbitrary arguments
        total = 0
        i = total_samples
        while i > 0:
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_roll in range(1, 11):
        ave = make_averaged(roll_dice, total_samples)(num_roll, dice)
        if ave > highest:
            highest = ave
            roll = num_roll
        elif ave == highest:
            roll = min(roll, num_roll)
    return roll
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    temp_roll = hefty_hogs_strategy(score, opponent_score, threshold, num_rolls)
    if temp_roll != num_rolls:
        return 0
    else:
        temp_score = hefty_hogs(score, opponent_score)
        if temp_score > 0 and (temp_score + score) % 10 == opponent_score % 10:
            return 0
        return num_rolls
    # END PROBLEM 11
